...
class CoolGameFunctionsTest(unittest.TestCase):
	...
	def test_eat_burgers(self):
		"""eat_burgers() have to return 'M'mm! That was excellent!' if number <= 3 """
	...
	def test_can_fly_anyone_else(self):
		...
		self.assertFalse(cool_game.can_fly('Bob'), False)
		self.assertFalse(cool_game.can_fly('Jim'), False)
		self.assertFalse(cool_game.can_fly('Kevin'), False)
